...
for i = 1:length(func_list)
    ...
    [IEIs,y_IEIs] = agg_func(@find_event_lengths, X, y);
...
title('Kernel density estimate of event length distribution')
xlabel('Event length')
ylabel('Probability density')
legend('show')
saveas(gcf,[directory,'KDE-length.pdf'])
...
for i = 1:length(func_list)
    ...
    alpha(0.5)
    legend('show')
    xlabel('Event length (ms)')
    ylabel('Number of events')
    title('Event length histogram')
    saveas(gcf,[directory,func2str(func_list{i}),'length-hist.pdf'])
end
